...
public class T_PickManager extends PlatoTest {
	...
	@Test
	public void pick() {
		Pick p1 = m.addPick(0, 0, 0, dirt);
		assertThat(m.getPick(0), is(p1));
	}

	@Test
	public void reset() {
		m.addPick(0, 0, 0, dirt);
		m.reset(3);
		for (int i = 0; i < 6; i++) {
			m.addPick(i, 0, 0, dirt);
		}
	...
	public void getPickAt() {
		Pick p = m.addPick(1, 0, 0, dirt);
		assertThat(m.getPickAt(1, 0, 0), is(p));
	}

	@Test
	public void clear() {
		m.addPick(1, 0, 0, dirt);
		m.addPick(2, 0, 0, dirt);
		m.clear();
	...
	public void getPickPoints() {
		Pick p1 = m.addPick(1, 0, 0, dirt);
		Pick p2 = m.addPick(2, 0, 0, dirt);
